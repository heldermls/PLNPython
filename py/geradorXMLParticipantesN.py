...
for line in lines:
    ...
    if not line or bool(re.match(r'\s\s*', line)):
        ...
        txtanos = ""
        tamanho = len(re.findall(r'\d\d\d\d', line))
        nparticipacoes = et.SubElement(participante, "nparticipacoes")
        nparticipacoes.text = str(tamanho)
f.close()
#prettify xml
...
with io.open("../xml/ParticipantesQ.xml", "w+", encoding="utf-8") as f:
    f.write(formatedXML)
